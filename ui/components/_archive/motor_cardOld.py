...
def _merge_output_blocks(outputs_by_block: dict, *names: str) -> list[dict]:
    merged = []
    for name in names:
        merged.extend(outputs_by_block.get(name, []) or [])
    return merged
...
def render_motor_card(motor_result: dict):
    ...
    with top_right:
        with st.container(border=True):
            ...
            st.write(f"**Salidas disparadas:** {len(all_outputs)}")
            st.write(f"**Reglas no resueltas:** {len(unknown_rules)}")

    if all_outputs:
        col_a, col_b = st.columns(2, gap="large")
        with col_a:
            _render_output_group(
                "Resumen ejecutivo",
                _merge_output_blocks(outputs_by_block, "executive_summary"),
            )
        with col_b:
            _render_output_group(
                "Interpretación clínica",
                _merge_output_blocks(outputs_by_block, "clinical_interpretation"),
            )
